...
class Decoder(object):
    ...
    def __init__(self, hidden_size, Ddim, dropout, config, initializer=lambda: None):
        self.hidden_size = hidden_size
        self.init_weights = initializer
        self.Ddim = Ddim
        self.dropout = dropout
    ...
    def decode(self, encoded_rep, q_rep, masks):
        ...
        #if self.config.mode == "transfer":
        #    logits = self.run_projection(outputs, 1, "SemEval_projection_transfer")
        #else:
...
class MatchLSTM(object):
    ...
    def __init__(self, flags, vocab, char_vocab, word_embedding):
        ...
        self.decoder = Decoder(self.config.hidden_layer*2 ,
                               self.Ddim, self.dropout, self.config)
        self._build_model()
        tvars = tf.trainable_variables()
        transfer_vars = [var for var in tvars if "SemEval_projection" in var.name or "embeddings" in var.name]
        if self.config.mode != "debug":
            if self.config.mode == "transfer":
                print("Num of transfer value:", len(transfer_vars))
                self.train_op = tf.train.AdamOptimizer(
                    learning_rate=self.config.learning_rate).minimize(self.loss, var_list=transfer_vars)
            else:
                self.train_op = tf.train.AdamOptimizer(
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("loss"):
            ...
            vars_SNLI = [v for v in vars
                         if 'bias' not in v.name and "embedd" not in v.name and "SQUAD" not in v.name and "SemEval" not in v.name]
            print("Number of parameter in SemEval task is {}".format(
                len(vars_SemEval)))
            print("Number of parameter in SQUAD task is {}".format(len(vars_SNLI)))
            lossL2_SemEval = tf.add_n([tf.nn.l2_loss(v)
                                       for v in vars_SemEval]) * 1e-5
            lossL2_SNLI = tf.add_n([tf.nn.l2_loss(v)
                                    for v in vars_SNLI]) * 1e-5
            #supervised_attn_loss = tf.nn.l2_loss(self.attention_fw - self.attention_bw) * 1e-6
            self.loss = self.loss #+ lossL2_SemEval
            self.loss_SNLI = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.y_SNLI,
                logits=logits_SNLI
            )) + lossL2_SNLI #+ supervised_attn_loss
